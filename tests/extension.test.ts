...
test("rellena los datos que están en la ficha", () => {
  const w = montar(`<form>
  ...
  (w as never as { rellenarFormulario: (f: unknown) => { escritos: number } }).rellenarFormulario(FICHA);
  assert.equal(val("#n"), "Ana Torres Gil");
  assert.equal(val("#e"), "ana@ejemplo.com");
  assert.equal(val("#t"), "+34 600 000 000");
});

test("NUNCA toca contraseñas ni documentos de identidad", () => {
  const w = montar(`<form>
  ...
  (w as never as { rellenarFormulario: (f: unknown) => unknown }).rellenarFormulario(FICHA);
  assert.equal(val("#p"), "");
  assert.equal(val("#d"), "");
  assert.equal(val("#c"), "");
});

test("NUNCA marca casillas ni botones de opción", () => {
  const w = montar(`<form>
...
test("encuentra la etiqueta aunque sea un hermano previo con formato", () => {
  const w = montar(`<form>
...
test("NO mete el correo en una pregunta sobre experiencia que mencione email", () => {
  const w = montar(`<form>
...
test("un desplegable con opción vacía se rellena", () => {
  const w = montar(`<form>
...
test("un desplegable sin opción vacía corrige su valor por defecto", () => {
  const w = montar(`<form>
...
test("un desplegable que el usuario ya eligió no se toca", () => {
  const w = montar(`<form>
    <label for="p">País</label>
...
test("recoge las preguntas abiertas y descarta los campos de datos", () => {
  const w = montar(`<form>
...
test("quita el prefijo 'Candidatura para' del título de la página", () => {
